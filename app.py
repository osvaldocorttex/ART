...
_SQL_ESCRITA_RE = re.compile(r"^\s*(INSERT|UPDATE|DELETE|REPLACE)\b", re.IGNORECASE)
_contexto_guarda_leitura = threading.local()


def _bypass_guarda_leitura_ativo():
    return bool(getattr(_contexto_guarda_leitura, "bypass", False))


@contextlib.contextmanager
def _sem_guarda_leitura():
    """Permite gravações de manutenção do sistema (init_db, versão) mesmo em sessão somente leitura."""
    anterior = _bypass_guarda_leitura_ativo()
    _contexto_guarda_leitura.bypass = True
    try:
        yield
    finally:
        _contexto_guarda_leitura.bypass = anterior


class _ConexaoComGuardaLeitura(sqlite3.Connection):
    def execute(self, sql, parameters=()):
        if (
            usuario_e_somente_leitura()
            and not _bypass_guarda_leitura_ativo()
            and _SQL_ESCRITA_RE.match(sql)
        ):
            st.error(
                "🔒 Este usuário tem acesso somente leitura e não pode gravar, alterar ou excluir dados."
            )
            st.stop()
...
def conn():
    c = sqlite3.connect(DB, check_same_thread=False, factory=_ConexaoComGuardaLeitura)
...
ABAS_SISTEMA = [
    ...
    ("movimento_viagens", "📌 Movimento Viagens"),
    ("viagens_executadas", "📋 Viagens Executadas"),
    ("analise", "📊 Análise"),
    ...
    ("metas", "🎯 Metas"),
    ("trocas", "🛢️ Trocas"),
    ("frete_liquido", "💵 Frete Líquido no Período"),
...
def usuario_e_somente_leitura():
    return bool(st.session_state.get("usuario_somente_leitura"))


def placas_liberadas_usuario():
    return list(st.session_state.get("usuario_placas_liberadas") or [])


def usuario_pode_ver_aba(aba_key):
    if not usuario_e_somente_leitura():
        return True
    return aba_key in (st.session_state.get("usuario_abas_liberadas") or [])


def filtrar_df_por_placas_liberadas(df, coluna="veiculo_placa"):
    if not usuario_e_somente_leitura():
        return df
    if df is None or df.empty or coluna not in df.columns:
        return df
    placas = {str(p).strip().upper() for p in placas_liberadas_usuario() if str(p or "").strip()}
    if not placas:
        return df.iloc[0:0]
    return df[df[coluna].fillna("").astype(str).str.strip().str.upper().isin(placas)]
...
def salvar_placas_usuario(usuario_id, placas):
    usuario_id = int(usuario_id)
    placas_norm = sorted({str(p).strip().upper() for p in (placas or []) if str(p or "").strip()})
    with conn() as c:
        c.execute("DELETE FROM usuario_placas_liberadas WHERE usuario_id=?", (usuario_id,))
        for placa in placas_norm:
            c.execute(
                "INSERT OR IGNORE INTO usuario_placas_liberadas (usuario_id, placa) VALUES (?, ?)",
                (usuario_id, placa),
            )


def carregar_placas_usuario(usuario_id):
    with conn() as c:
        rows = c.execute(
            "SELECT placa FROM usuario_placas_liberadas WHERE usuario_id=? ORDER BY placa",
            (int(usuario_id),),
        ).fetchall()
    return [r["placa"] for r in rows]


def salvar_abas_usuario(usuario_id, abas):
    usuario_id = int(usuario_id)
    chaves_validas = {chave for chave, _ in ABAS_SISTEMA}
    abas_norm = sorted({str(a).strip() for a in (abas or []) if str(a or "").strip() in chaves_validas})
    with conn() as c:
        c.execute("DELETE FROM usuario_abas_liberadas WHERE usuario_id=?", (usuario_id,))
        for aba_key in abas_norm:
            c.execute(
                "INSERT OR IGNORE INTO usuario_abas_liberadas (usuario_id, aba_key) VALUES (?, ?)",
                (usuario_id, aba_key),
            )


def carregar_abas_usuario(usuario_id):
    with conn() as c:
        rows = c.execute(
            "SELECT aba_key FROM usuario_abas_liberadas WHERE usuario_id=? ORDER BY aba_key",
            (int(usuario_id),),
        ).fetchall()
    return [r["aba_key"] for r in rows]


def cadastrar_usuario_sistema(
    usuario,
    senha,
    nome_estacao=None,
    is_admin=False,
    ...
    somente_leitura = bool(somente_leitura) and not is_admin

    try:
        with conn() as c:
            cur = c.execute(
                """INSERT INTO usuarios_sistema (usuario, senha_hash, data_cadastro, ativo, is_admin, somente_leitura)
                   VALUES (?, ?, ?, 1, ?, ?)""",
                (
                    usuario_limpo,
        ...
        if somente_leitura:
            salvar_placas_usuario(usuario_id, placas_liberadas)
            salvar_abas_usuario(usuario_id, abas_liberadas)
...
def validar_usuario_sistema(usuario, senha):
    ...
    with conn() as c:
        row = c.execute(
            "SELECT id, usuario, senha_hash, is_admin, somente_leitura FROM usuarios_sistema WHERE lower(usuario)=? AND ativo=1",
            (usuario_limpo,),
        ).fetchone()
...
def atualizar_usuario_sistema(
    ...
    leitura_int = 1 if (somente_leitura and not is_admin) else 0
        ...
        with conn() as c:
            if nova_senha:
                if len(str(nova_senha)) < 4:
                    return False, "A senha deve ter pelo menos 4 caracteres."
                c.execute(
                    "UPDATE usuarios_sistema SET usuario=?, senha_hash=?, is_admin=?, ativo=?, somente_leitura=? WHERE id=?",
                    (usuario_limpo, gerar_hash_senha(nova_senha), admin_int, ativo_int, leitura_int, usuario_id),
                )
            else:
                c.execute(
                    "UPDATE usuarios_sistema SET usuario=?, is_admin=?, ativo=?, somente_leitura=? WHERE id=?",
                    (usuario_limpo, admin_int, ativo_int, leitura_int, usuario_id),
                )
        salvar_placas_usuario(usuario_id, placas_liberadas if leitura_int else [])
        salvar_abas_usuario(usuario_id, abas_liberadas if leitura_int else [])
...
def listar_usuarios_sistema():
    with conn() as c:
        rows = c.execute(
            """SELECT id, usuario, is_admin, ativo, data_cadastro, somente_leitura
...
def proteger_abertura_sistema():
    ...
    if primeiro_acesso:
        ...
        if ok:
            ...
            st.session_state.usuario_somente_leitura = False
            st.session_state.usuario_placas_liberadas = []
            st.session_state.usuario_abas_liberadas = []
            st.success(msg)
            st.rerun()
        st.error(msg)
        st.stop()
    ...
    usuario_somente_leitura = (not usuario_admin) and bool(int(row_usuario["somente_leitura"] or 0) == 1)
    st.session_state.acesso_liberado = True
    st.session_state.usuario_logado = row_usuario["usuario"]
    st.session_state.usuario_admin = usuario_admin
    st.session_state.usuario_somente_leitura = usuario_somente_leitura
    st.session_state.usuario_placas_liberadas = (
        carregar_placas_usuario(row_usuario["id"]) if usuario_somente_leitura else []
    )
    st.session_state.usuario_abas_liberadas = (
        carregar_abas_usuario(row_usuario["id"]) if usuario_somente_leitura else []
    )
    st.rerun()
...
def somar_um_mes(data_ref):
    """Soma 1 mês a uma data, ajustando para o último dia do mês quando necessário
    (ex.: 31/01 -> 28 ou 29/02)."""
    if data_ref is None:
        return None
    if isinstance(data_ref, str):
        iso = normalizar_data_iso(data_ref)
        if not iso:
            return None
        data_ref = datetime.strptime(iso, "%Y-%m-%d").date()
    ano_novo = data_ref.year + (1 if data_ref.month == 12 else 0)
    mes_novo = 1 if data_ref.month == 12 else data_ref.month + 1
    ultimo_dia_mes_novo = calendar.monthrange(ano_novo, mes_novo)[1]
    dia_novo = min(data_ref.day, ultimo_dia_mes_novo)
...
def movimentos_fornecedor(c, fornecedor_id):
    ...
    row_nome_forn = c.execute("SELECT nome FROM fornecedores WHERE id=?", (fornecedor_id,)).fetchone()
    nome_forn = str(row_nome_forn["nome"] or "").strip() if row_nome_forn else ""
    if nome_forn:
        qtd_cp = c.execute("SELECT COUNT(*) FROM contas_pagar WHERE fornecedor=?", (nome_forn,)).fetchone()[0]
        if qtd_cp:
    ...
    with conn() as c:
        return pd.read_sql(
            """SELECT COALESCE(NULLIF(TRIM(veiculo_placa), ''), 'GERAL') AS veiculo_placa,
                      ...
                      seguro, seguro_vida_motorista, financiamento, pagto_ipva, valor_inss, valor_acerto_folha, adiantamento_salario, cmp_custo_escritorio, vl_custo_rastreador, imposto_pct, valor_frete_mensal_fixo,
...
@cache_data_compat(ttl=15, show_spinner=False)
def _carregar_manutencoes_periodo_raw(data_ini_iso: str, data_fim_iso: str):
    with conn() as c:
        return pd.read_sql(
            """SELECT m.*, f.nome as fornecedor_nome FROM manutencoes m
...
@cache_data_compat(ttl=30, show_spinner=False)
def _carregar_placas_manutencao_raw():
    with conn() as c:
        return pd.read_sql(
...
@cache_data_compat(ttl=20, show_spinner=False)
def _carregar_controle_trocas_raw():
    with conn() as c:
        return pd.read_sql("SELECT * FROM controle_trocas ORDER BY data_servico DESC", c)


def limpar_cache_trocas():
    func = globals().get("_carregar_controle_trocas_raw")
...
@cache_data_compat(ttl=15, show_spinner=False)
def _carregar_contas_pagar_raw():
    with conn() as c:
        return pd.read_sql("SELECT * FROM contas_pagar ORDER BY data_vencimento ASC, id DESC", c)


@cache_data_compat(ttl=15, show_spinner=False)
def _carregar_contas_receber_raw():
    with conn() as c:
        return pd.read_sql("SELECT * FROM contas_receber ORDER BY data_vencimento ASC, id DESC", c)


@cache_data_compat(ttl=15, show_spinner=False)
def _carregar_abastecimentos_periodo_raw(data_ini_iso: str, data_fim_iso: str):
    with conn() as c:
        return pd.read_sql(
            """SELECT *
               FROM abastecimentos
...
def limpar_cache_manutencao():
    for nome_func in ("_carregar_manutencoes_periodo_raw", "_carregar_placas_manutencao_raw"):
...
def limpar_cache_app():
    funcoes_cache = [
        ...
        "_carregar_manutencoes_periodo_raw",
        "_carregar_placas_manutencao_raw",
        "_carregar_controle_trocas_raw",
        "_carregar_contas_pagar_raw",
        "_carregar_contas_receber_raw",
        "_carregar_abastecimentos_periodo_raw",
    ]
...
def registrar_versao_sistema():
    ...
    with _sem_guarda_leitura(), conn() as c:
        c.execute(
...
def init_db():
    with _sem_guarda_leitura(), conn() as c:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS usuario_placas_liberadas (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            usuario_id INTEGER NOT NULL,
            placa TEXT NOT NULL,
            UNIQUE(usuario_id, placa)
        )""")
        c.execute("""CREATE TABLE IF NOT EXISTS usuario_abas_liberadas (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            usuario_id INTEGER NOT NULL,
            aba_key TEXT NOT NULL,
            UNIQUE(usuario_id, aba_key)
        )""")
        ...
        if "somente_leitura" not in colunas_usuarios:
            c.execute("ALTER TABLE usuarios_sistema ADD COLUMN somente_leitura INTEGER DEFAULT 0")
        c.execute(
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS parametros (
            ...
            adiantamento_salario REAL DEFAULT 0.0,
...
# --- ATUALIZAÇÃO AUTOMÁTICA DAS COLUNAS MANUTENÇÃO ---
        ...
        if "adiantamento_salario" not in colunas_param:
            c.execute("ALTER TABLE parametros ADD COLUMN adiantamento_salario REAL DEFAULT 0.0")
        ...
        if "adiantamento_salario" not in colunas_hist_param:
            c.execute("ALTER TABLE parametros_historico ADD COLUMN adiantamento_salario REAL DEFAULT 0.0")
        ...
        if precisa_recriar_hist_param:
            ...
            c.execute("""INSERT OR REPLACE INTO parametros_historico_nova (
                            ...
                            pagto_ipva, valor_inss, valor_acerto_folha, adiantamento_salario, cmp_custo_escritorio, vl_custo_rastreador, imposto_pct,
                         ...
                         SELECT id, COALESCE(NULLIF(TRIM(veiculo_placa), ''), 'GERAL'), vigencia_data,
                                ...
                                seguro_vida_motorista, financiamento, pagto_ipva, COALESCE(valor_inss, 0.0), COALESCE(valor_acerto_folha, 0.0), COALESCE(adiantamento_salario, 0.0), cmp_custo_escritorio,
...
if usuario_e_somente_leitura():
    _placas_liberadas_sessao = {p.strip().upper() for p in placas_liberadas_usuario() if str(p or "").strip()}
    veiculos_db = [v for v in veiculos_db if str(v.get("placa") or "").strip().upper() in _placas_liberadas_sessao]
...
if not st.session_state.popup_trocas_exibido:
    try:
        ...
        with conn() as c:
            ...
            df_alerta_trocas = filtrar_df_por_placas_liberadas(df_alerta_trocas, "veiculo_placa")
...
df_db = filtrar_df_por_placas_liberadas(df_db, "veiculo_placa")
if not df_db.empty:
...
if usuario_e_somente_leitura():
    _placas_txt_ro = ", ".join(placas_liberadas_usuario()) or "nenhuma placa liberada"
    st.warning(f"🔒 Acesso somente leitura — placas liberadas: **{_placas_txt_ro}**. Não é possível incluir, alterar ou excluir dados.")
    ...
    if not st.session_state.get("usuario_admin"):
        ...
        with tab_incluir_usuario:
            ...
            with st.form("form_admin_cadastrar_usuario"):
                ...
                novo_somente_leitura = st.checkbox(
                    "Somente leitura (restrito por placa e abas)", value=False, key="admin_novo_somente_leitura"
                )
                st.caption("Válido apenas quando o usuário não é administrador.")
                novas_placas = st.multiselect(
                    "Placas liberadas para este usuário",
                    apenas_placas,
                    format_func=rotulo_placa_com_descricao,
                    key="admin_novo_placas",
                )
                novas_abas = st.multiselect(
                    "Abas liberadas para este usuário",
                    [chave for chave, _ in ABAS_SISTEMA],
                    format_func=lambda chave: dict(ABAS_SISTEMA).get(chave, chave),
                    key="admin_novo_abas",
                )
                if st.form_submit_button("➕ Incluir usuário", type="primary"):
                    ok, msg = cadastrar_usuario_sistema(
                        ...
                        is_admin=novo_admin,
                        liberar_estacao=False,
                        somente_leitura=novo_somente_leitura,
                        placas_liberadas=novas_placas,
                        abas_liberadas=novas_abas,
                    )
                    if ok:
                        alerta_gravado(msg)
                        st.rerun()
        ...
        with tab_alterar_usuario:
            ...
            if usuarios_opcoes:
                ...
                placas_atuais_alt = carregar_placas_usuario(int(row_alt["id"]))
                abas_atuais_alt = carregar_abas_usuario(int(row_alt["id"]))
                with st.form(f"form_admin_alterar_usuario_{int(row_alt['id'])}"):
                    usuario_alt_nome = st.text_input("Usuário", value=str(row_alt["usuario"] or ""), key=f"admin_usuario_alt_nome_{int(row_alt['id'])}")
                    senha_alt = st.text_input("Nova senha (deixe em branco para manter)", type="password", key=f"admin_usuario_alt_senha_{int(row_alt['id'])}")
                    admin_alt = st.checkbox("Administrador", value=bool(int(row_alt["is_admin"] or 0) == 1), key=f"admin_usuario_alt_admin_{int(row_alt['id'])}")
                    ativo_alt = st.checkbox("Ativo", value=bool(int(row_alt["ativo"] or 0) == 1), key=f"admin_usuario_alt_ativo_{int(row_alt['id'])}")
                    somente_leitura_alt = st.checkbox(
                        "Somente leitura (restrito por placa e abas)",
                        value=bool(int(row_alt["somente_leitura"] or 0) == 1),
                        key=f"admin_usuario_alt_somente_leitura_{int(row_alt['id'])}",
                    )
                    st.caption("Válido apenas quando o usuário não é administrador.")
                    placas_alt = st.multiselect(
                        "Placas liberadas para este usuário",
                        apenas_placas,
                        default=[p for p in placas_atuais_alt if p in apenas_placas],
                        format_func=rotulo_placa_com_descricao,
                        key=f"admin_usuario_alt_placas_{int(row_alt['id'])}",
                    )
                    abas_alt = st.multiselect(
                        "Abas liberadas para este usuário",
                        [chave for chave, _ in ABAS_SISTEMA],
                        default=[a for a in abas_atuais_alt if a in dict(ABAS_SISTEMA)],
                        format_func=lambda chave: dict(ABAS_SISTEMA).get(chave, chave),
                        key=f"admin_usuario_alt_abas_{int(row_alt['id'])}",
                    )
                    if st.form_submit_button("✏️ Alterar usuário", type="primary"):
                        ok, msg = atualizar_usuario_sistema(
                            ...
                            is_admin=admin_alt,
                            ativo=ativo_alt,
                            somente_leitura=somente_leitura_alt,
                            placas_liberadas=placas_alt,
                            abas_liberadas=abas_alt,
                        )
                        if ok:
                            alerta_gravado(msg)
                            st.rerun()
            ...
            [
                {
                    ...
                    "Somente leitura": "Sim" if int(r["somente_leitura"] or 0) == 1 else "Não",
                    "Placas liberadas": ", ".join(carregar_placas_usuario(int(r["id"]))) or "-",
                    "Abas liberadas": ", ".join(
                        dict(ABAS_SISTEMA).get(a, a) for a in carregar_abas_usuario(int(r["id"]))
                    ) or "-",
...
with aba_home:
    if usuario_pode_ver_aba("dashboard"):
        st.markdown(
            """
            <style>
            .stApp {
        ...
        if df_db.empty:
            ...
            df_abs_dash = filtrar_df_por_placas_liberadas(df_abs_dash, "veiculo_placa")
            ...
            # Descontos de INSS e Acerto Folha sobre o custo de motorista, igual à aba Análise,
            # para que o Lucro Operacional seja consistente entre as duas abas.
            t_inss_dash = valor_mensal_rateado_periodo("valor_inss", filtro_ini, filtro_fim)
            t_acerto_folha_dash = valor_mensal_rateado_periodo("valor_acerto_folha", filtro_ini, filtro_fim)
            incluir_inss_dash = st.session_state.get("ana_descontar_valor_inss", True)
            incluir_acerto_folha_dash = st.session_state.get("ana_descontar_valor_acerto_folha", True)
            desconto_inss_dash = t_inss_dash if incluir_inss_dash else 0.0
            desconto_acerto_folha_dash = t_acerto_folha_dash if incluir_acerto_folha_dash else 0.0
            desconto_folha_total_dash = desconto_inss_dash + desconto_acerto_folha_dash
            if total_receita_final > 0:
                df_dash["desconto_folha_rateado"] = desconto_folha_total_dash * proporcao_receita
            else:
                df_dash["desconto_folha_rateado"] = 0.0
            ...
            df_dash["lucro_operacional"] = (
                ...
                + df_dash["desconto_folha_rateado"]
            ).fillna(0.0)
            ...
            if desconto_folha_total_dash > 0:
                st.caption(
                    f"Lucro Operacional já descontado de: INSS {brl(desconto_inss_dash)} e Acerto Folha {brl(desconto_acerto_folha_dash)} "
                    "(mesmo critério da aba Análise, conforme opções marcadas por lá)."
                )
            ...
            if not df_tend.empty:
                ...
                df_tend_g = (
                    ...
                    .agg(
                        ...
                        desconto_folha_rateado=("desconto_folha_rateado", "sum"),
                    )
                    .sort_values("mes")
                )
                ...
                df_tend_g["custos"] = (
                    ...
                    - df_tend_g["desconto_folha_rateado"]
                )
    ...
    # Abas 1 a 12 permanecem iguais
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba1:
    if usuario_pode_ver_aba("movimento_viagens"):
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba2:
    if usuario_pode_ver_aba("viagens_executadas"):
        if not df_db.empty:
            ...
            if "dias_gastos" not in df_exibir.columns:
                df_exibir["dias_gastos"] = 0
            ...
            df_exibir["dias_gastos"] = pd.to_numeric(df_exibir["dias_gastos"], errors="coerce").fillna(0).astype(int)
            ...
            total_dias_gastos = int(pd.to_numeric(df_exibir["dias_gastos"], errors="coerce").fillna(0).sum())
            ...
            m12.metric("Dias Gastos", total_dias_gastos)
            ...
            with tab_grid_exec:
                ...
                if st.session_state.viagens_exec_id_editando is not None:
                    ...
                    if reg_sel.empty:
                        ...
                        if origem_ed:
                            with conn() as c:
                                rows_destino_ed = c.execute(
                                    "SELECT destino FROM rotas WHERE origem=? UNION SELECT origem FROM rotas WHERE destino=?",
                                    (origem_ed, origem_ed),
                                ).fetchall()
                            opcoes_destino_ed = sorted({
                                str(row[0]).strip() for row in rows_destino_ed
                                if row[0] and str(row[0]).strip() != origem_ed
                            })
                        destino_padrao_ed = destino_atual_ed if origem_ed == origem_atual_ed else None
                        if destino_padrao_ed and destino_padrao_ed not in opcoes_destino_ed:
                            opcoes_destino_ed = [destino_padrao_ed] + opcoes_destino_ed
                        destino_key_ed = f"exec_destino_ed_{int(r['id'])}"
                        if destino_key_ed not in st.session_state:
                            st.session_state[destino_key_ed] = destino_padrao_ed
                        elif st.session_state[destino_key_ed] not in opcoes_destino_ed:
                            st.session_state[destino_key_ed] = destino_padrao_ed if destino_padrao_ed in opcoes_destino_ed else None
                        destino_ed = ce5.selectbox(
                            "Destino",
                            options=opcoes_destino_ed,
                            index=None,
                            key=destino_key_ed,
                        )
                        if origem_ed and not opcoes_destino_ed:
                            ce5.caption("Nenhuma rota cadastrada para esta origem em KM Rotas.")
                        ...
                        with st.form("form_edicao_viagem_exec"):
                            ...
                            cliente_atual_ed = str(r["cliente"] or "").strip()
                            opcoes_cliente_ed = list(lista_clientes) if isinstance(lista_clientes, list) else []
                            if cliente_atual_ed and cliente_atual_ed not in opcoes_cliente_ed:
                                opcoes_cliente_ed = [cliente_atual_ed] + opcoes_cliente_ed
                            if opcoes_cliente_ed:
                                idx_cliente_ed = opcoes_cliente_ed.index(cliente_atual_ed) if cliente_atual_ed in opcoes_cliente_ed else 0
                                cliente_ed = ce3.selectbox(
                                    "Cliente",
                                    options=opcoes_cliente_ed,
                                    index=idx_cliente_ed,
                                    key=f"exec_cliente_ed_{int(r['id'])}",
                                )
                            else:
                                cliente_ed = ce3.text_input("Cliente", value=cliente_atual_ed)
                            ...
                            ce_dias_gastos, _ = st.columns([1, 3])
                            dias_gastos_ed = ce_dias_gastos.number_input(
                                "Dias Gastos",
                                min_value=0,
                                value=int(r.get("dias_gastos", 0) or 0),
                                step=1,
                            )
                        ...
                        if btn_atualizar_viagem:
                            if not origem_ed.strip() or not str(destino_ed or "").strip() or not placa_ed.strip():
                                ...
                                with conn() as c:
                                    c.execute(
                                        """UPDATE viagens SET
                                           ...
                                           nf=?, veiculo_placa=?, qtd_viagens=?, dias_gastos=?
                                           WHERE id=?""",
                                        (
                                            data_ed.isoformat(),
                                            cliente_ed.strip(),
                        ...
                        if btn_excluir_viagem:
                            st.session_state["viagem_exec_excluir_confirmar_id"] = int(st.session_state.viagens_exec_id_editando)
                            st.rerun()

                        if st.session_state.get("viagem_exec_excluir_confirmar_id") == int(r["id"]):
                            st.warning(f"Confirma a exclusão da viagem ID {int(r['id'])}? Essa ação não pode ser desfeita.")
                            col_conf_exc, col_cancel_exc = st.columns(2)
                            if col_conf_exc.button(
                                "✅ Confirmar exclusão", type="primary", use_container_width=True,
                                key=f"btn_viagem_exec_confirmar_exclusao_{int(r['id'])}",
                            ):
                                with conn() as c:
                                    c.execute("DELETE FROM viagens WHERE id=?", (int(st.session_state.viagens_exec_id_editando),))
                                limpar_cache_viagens()
                                st.session_state["viagem_exec_excluir_confirmar_id"] = None
                                alerta_gravado("✅ Viagem excluída com sucesso!")
                                st.session_state.viagens_exec_id_editando = None
                                st.rerun()
                            if col_cancel_exc.button(
                                "❌ Cancelar", use_container_width=True,
                                key=f"btn_viagem_exec_cancelar_exclusao_{int(r['id'])}",
                            ):
                                st.session_state["viagem_exec_excluir_confirmar_id"] = None
                                st.rerun()
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba3:
    if usuario_pode_ver_aba("analise"):
        if not df_db.empty:
            df_ana = df_db.copy()
            ...
            if "arla" not in df_ana.columns:
                df_ana["arla"] = 0.0
            if "consumo_arla" not in df_ana.columns:
                df_ana["consumo_arla"] = 0.0
            ...
            df_ana["arla"] = pd.to_numeric(df_ana["arla"], errors="coerce").fillna(0.0)
            df_ana["consumo_arla"] = pd.to_numeric(df_ana["consumo_arla"], errors="coerce").fillna(0.0)
            ...
            consumo_diesel_valido_ana = df_ana["consumo"] > 0
            df_ana["litros_diesel"] = 0.0
            df_ana.loc[consumo_diesel_valido_ana, "litros_diesel"] = (
                df_ana.loc[consumo_diesel_valido_ana, "km_total"] / df_ana.loc[consumo_diesel_valido_ana, "consumo"]
            )
            df_ana["custo_diesel"] = (df_ana["litros_diesel"] * df_ana["diesel"]).fillna(0.0)

            consumo_arla_valido_ana = df_ana["consumo_arla"] > 0
            df_ana["litros_arla"] = 0.0
            df_ana.loc[consumo_arla_valido_ana, "litros_arla"] = (
                df_ana.loc[consumo_arla_valido_ana, "km_total"] / df_ana.loc[consumo_arla_valido_ana, "consumo_arla"]
            )
            df_ana["custo_arla"] = (df_ana["litros_arla"] * df_ana["arla"]).fillna(0.0)

            # Usar gasto real de Arla (abastecimentos) quando disponível, igual à aba Dashboard Executivo,
            # para que o Lucro Líquido seja consistente entre as duas abas.
            with conn() as c:
            ...
            df_abs_ana = filtrar_df_por_placas_liberadas(df_abs_ana, "veiculo_placa")
            ...
            if not df_abs_ana.empty:
                ...
                custo_arla_real_ana = float(
                    df_abs_ana[df_abs_ana["tipo_combustivel"].str.contains("ARLA", na=False)]["total_gasto"].sum()
                )
                total_km_rateio_arla_ana = float(df_ana["km_total"].sum())
                if custo_arla_real_ana > 0 and total_km_rateio_arla_ana > 0:
                    df_ana["custo_arla"] = (custo_arla_real_ana * (df_ana["km_total"] / total_km_rateio_arla_ana)).fillna(0.0)
                # else: mantém custo_arla calculado por arla/consumo_arla das viagens

            t_litros_diesel_ana = float(df_ana["litros_diesel"].sum())
            t_litros_arla_ana = float(df_ana["litros_arla"].sum())
            t_valor_arla_ana = float(df_ana["custo_arla"].sum())
            ...
            t_inss_ana = valor_mensal_rateado_periodo("valor_inss", filtro_ini, filtro_fim)
            t_acerto_folha_ana = valor_mensal_rateado_periodo("valor_acerto_folha", filtro_ini, filtro_fim)
            desc_ana_col1, desc_ana_col2 = st.columns(2)
            incluir_inss_ana = desc_ana_col1.checkbox(
                "Descontar Valor INSS",
                value=True,
                key="ana_descontar_valor_inss",
            )
            incluir_acerto_folha_ana = desc_ana_col2.checkbox(
                "Descontar Valor Acerto Folha",
                value=True,
                key="ana_descontar_valor_acerto_folha",
            )
            desconto_inss_ana = t_inss_ana if incluir_inss_ana else 0.0
            desconto_acerto_folha_ana = t_acerto_folha_ana if incluir_acerto_folha_ana else 0.0
            t_mot_total = t_mot_fixo_rateado + t_comis - desconto_inss_ana - desconto_acerto_folha_ana
            ...
            custo_total_periodo = t_rec - lucro
            custo_bruto_por_km = (t_rec / t_km) if t_km > 0 else 0.0
            custo_liquido_por_km = (lucro / t_km) if t_km > 0 else 0.0

            def fmt_valor_por_km(valor: float) -> str:
                return (
                    f"R$ {valor:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".") + "/KM"
                )

            m1, m2, m3, m4, m5 = st.columns(5)
            m1.metric("Total KM", f"{format_br(t_km, casas_decimais=0)} KM")
            m2.metric("Total Valor Diesel", brl(t_die))
            m3.metric("Valor Total Arla", brl(t_valor_arla_ana))
            m4.metric("Faturamento", brl(t_rec))
            m5.metric("Custo Total", brl(custo_total_periodo))
        
            m6, m7, m8, m9, m10 = st.columns(5)
            m6.metric("Total Bruto/KM", fmt_valor_por_km(custo_bruto_por_km))
            m7.metric("Total Líquido/KM", fmt_valor_por_km(custo_liquido_por_km))
            m8.metric("Litros Diesel", f"{t_litros_diesel_ana:.2f} L")
            m9.metric("Litros Arla", f"{t_litros_arla_ana:.2f} L")
            m10.metric("Lucro Líquido", brl(lucro))
            ...
            st.caption(
                f"Motorista (Rateado) já descontado de: INSS {brl(desconto_inss_ana)} e Acerto Folha {brl(desconto_acerto_folha_ana)} "
                "(valores dos Parâmetros, rateados no período)."
            )
    ...
    # ==================================================================================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba4:
    if usuario_pode_ver_aba("manutencao"):
        ...
        df_placas_manut = filtrar_df_por_placas_liberadas(_carregar_placas_manutencao_raw(), "placa")
        ...
        df_m = _carregar_manutencoes_periodo_raw(filtro_ini.isoformat(), filtro_fim.isoformat()).copy()
        ...
        if fornecedor_manut_selecionado != "Todos os fornecedores":
            ...
            df_m = df_m[df_m["oficina_id"] == fornecedor_id]
        ...
        if placas_manut_selecionadas:
            placas_manut_upper = {str(p).strip().upper() for p in placas_manut_selecionadas}
            df_m = df_m[df_m["veiculo_placa"].fillna("").astype(str).str.strip().str.upper().isin(placas_manut_upper)]
        df_m = filtrar_df_por_placas_liberadas(df_m, "veiculo_placa")
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba5:
    if usuario_pode_ver_aba("cadastro"):
        ...
        if not df_oficinas.empty:
            ...
            if col_btn2.button("🔴 Excluir Oficinas Selecionadas", type="primary", key="btn_del_oficina"):
                ...
                if ids_para_excluir:
                    st.session_state["oficinas_excluir_confirmar_ids"] = ids_para_excluir
                    st.rerun()
                else:
                    st.info("Marque a lixeira (🗑️) das oficinas que deseja remover.")

            if st.session_state.get("oficinas_excluir_confirmar_ids"):
                qtd_of_excluir = len(st.session_state["oficinas_excluir_confirmar_ids"])
                st.warning(f"Confirma a exclusão de {qtd_of_excluir} oficina(s)? Essa ação não pode ser desfeita.")
                col_of_conf, col_of_cancel = st.columns(2)
                if col_of_conf.button("✅ Confirmar exclusão", type="primary", use_container_width=True, key="btn_oficina_confirmar_exclusao"):
                    with conn() as c:
                        for id_of in st.session_state["oficinas_excluir_confirmar_ids"]:
                            c.execute("DELETE FROM oficinas WHERE id=?", (id_of,))
                    st.session_state["oficinas_excluir_confirmar_ids"] = None
                    st.warning("Oficinas removidas.")
                    st.rerun()
                if col_of_cancel.button("❌ Cancelar", use_container_width=True, key="btn_oficina_cancelar_exclusao"):
                    st.session_state["oficinas_excluir_confirmar_ids"] = None
                    st.rerun()
        else:
        ...
        if not df_cidades.empty:
            ...
            if st.button("🗑️ Excluir Cidade", key="btn_cidade_excluir"):
                ...
                if qtd_mov_cidade:
                    ...
                    st.session_state["cidade_excluir_confirmar_id"] = int(id_cidade_excluir)
                    st.rerun()

            if st.session_state.get("cidade_excluir_confirmar_id") == id_cidade_excluir:
                nome_cidade_confirmar = str(df_cidades.loc[df_cidades["id"] == id_cidade_excluir, "nome"].iloc[0])
                st.warning(f"Confirma a exclusão da cidade '{nome_cidade_confirmar}'?")
                col_cid_conf, col_cid_cancel = st.columns(2)
                if col_cid_conf.button("✅ Confirmar exclusão", type="primary", use_container_width=True, key="btn_cidade_confirmar_exclusao"):
                    with conn() as c:
                        c.execute("DELETE FROM cidades WHERE id=?", (int(id_cidade_excluir),))
                    limpar_cache_bootstrap()
                    st.session_state["cidade_excluir_confirmar_id"] = None
                    alerta_gravado("✅ Cidade excluída com sucesso!")
                    st.rerun()
                if col_cid_cancel.button("❌ Cancelar", use_container_width=True, key="btn_cidade_cancelar_exclusao"):
                    st.session_state["cidade_excluir_confirmar_id"] = None
                    st.rerun()
        else:
...
with aba7:
    if usuario_pode_ver_aba("cadastro"):
                ...
                if reg_rota_sel.empty:
                    ...
                    if btn_excluir_rota:
                        ...
                        if check["total"] > 0:
                            ...
                            st.session_state["rota_excluir_confirmar_id"] = int(r["id"])
                            st.rerun()

                    if st.session_state.get("rota_excluir_confirmar_id") == int(r["id"]):
                        st.warning(f"Confirma a exclusão da rota {r['origem']} → {r['destino']}?")
                        col_rota_conf, col_rota_cancel = st.columns(2)
                        if col_rota_conf.button(
                            "✅ Confirmar exclusão", type="primary", use_container_width=True,
                            key=f"btn_rota_confirmar_exclusao_{int(r['id'])}",
                        ):
                            with conn() as c:
                                c.execute("DELETE FROM rotas WHERE id=?", (int(st.session_state.rota_id_editando),))
                            st.session_state["rota_excluir_confirmar_id"] = None
                            alerta_gravado("✅ Rota excluída com sucesso!")
                            st.session_state.rota_id_editando = None
                            st.rerun()
                        if col_rota_cancel.button(
                            "❌ Cancelar", use_container_width=True,
                            key=f"btn_rota_cancelar_exclusao_{int(r['id'])}",
                        ):
                            st.session_state["rota_excluir_confirmar_id"] = None
                            st.rerun()
        else:
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba8:
    if usuario_pode_ver_aba("relatorio"):
        ...
        valor_adiantamento_salario_rel = valor_parametro_relatorio_sem_rateio("adiantamento_salario")
        ...
        incluir_adiantamento_salario_rel = desc_col3.checkbox(
            "Descontar Adiantamento Salário",
            value=True,
            key="rel_descontar_adiantamento_salario",
        )
        ...
        if not df_db.empty:
            ...
            desconto_adiantamento_salario_rel = valor_adiantamento_salario_rel if incluir_adiantamento_salario_rel else 0.0
            total_descontos_folha_rel = desconto_inss_rel + desconto_acerto_folha_rel + desconto_adiantamento_salario_rel
            ...
            d3.metric("Adiantamento Salário", brl(desconto_adiantamento_salario_rel))
                ...
                html += f"""
                    ...
                    <div class="container-resumo">
                        <div class="tot">
                            ...
                            <div class="ln"><span>(-) Adiantamento Salário:</span> <span>{brl(desconto_adiantamento_salario_rel)}</span></div>
    ...
    # ==================================================================================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba9:
    if usuario_pode_ver_aba("abastecimento"):
        ...
        with st.expander("➕ Cadastro de Abastecimento", expanded=expandir_cadastro_abs):
            ...
            with st.form("form_inclusao_abastecimento", clear_on_submit=False):
                ...
                local_padrao_abs = str(abs_form_defaults.get("local") or "").strip()
                opcoes_local_abs = [str(f["nome"]).strip() for f in fornecedores_db if str(f.get("nome") or "").strip()]
                if local_padrao_abs and local_padrao_abs not in opcoes_local_abs:
                    opcoes_local_abs = [local_padrao_abs] + opcoes_local_abs
                if opcoes_local_abs:
                    idx_local_abs = opcoes_local_abs.index(local_padrao_abs) if local_padrao_abs in opcoes_local_abs else None
                    local_abs = col_c.selectbox(
                        "Local do Abastecimento",
                        options=opcoes_local_abs,
                        index=idx_local_abs,
                        placeholder="Selecione o fornecedor",
                        key=f"abs_local_incluir_{abs_form_nonce}",
                    )
                else:
                    local_abs = col_c.text_input("Local do Abastecimento", value=local_padrao_abs, key=f"abs_local_incluir_{abs_form_nonce}")
                ...
                if st.form_submit_button("💾 Gravar", use_container_width=True, type="primary", key="btn_abastecimento_incluir_gravar"):
                    if not placa_abs.strip():
                        ...
                        st.session_state["abs_expandir_cadastro"] = True
                        st.rerun()
                    elif not str(local_abs or "").strip():
        ...
        df_abs = _carregar_abastecimentos_periodo_raw(filtro_ini.isoformat(), filtro_fim.isoformat())
        df_abs = filtrar_df_por_placas_liberadas(df_abs, "veiculo_placa")
                ...
                if st.session_state.abastecimento_id_editando is not None:
                    ...
                    if registro_sel.empty:
                        ...
                        with st.form("form_edicao_abastecimento"):
                            ...
                            local_atual_ed_abs = str(r["local"] or "").strip()
                            opcoes_local_ed_abs = [str(f["nome"]).strip() for f in fornecedores_db if str(f.get("nome") or "").strip()]
                            if local_atual_ed_abs and local_atual_ed_abs not in opcoes_local_ed_abs:
                                opcoes_local_ed_abs = [local_atual_ed_abs] + opcoes_local_ed_abs
                            if opcoes_local_ed_abs:
                                idx_local_ed_abs = opcoes_local_ed_abs.index(local_atual_ed_abs) if local_atual_ed_abs in opcoes_local_ed_abs else 0
                                local_ed = c3.selectbox(
                                    "Local do Abastecimento",
                                    options=opcoes_local_ed_abs,
                                    index=idx_local_ed_abs,
                                    key=f"abs_local_ed_{int(st.session_state.abastecimento_id_editando)}",
                                )
                            else:
                                local_ed = c3.text_input("Local do Abastecimento", value=local_atual_ed_abs)
                        ...
                        if btn_excluir:
                            st.session_state["abastecimento_excluir_confirmar_id"] = int(st.session_state.abastecimento_id_editando)
                            st.rerun()

                        if st.session_state.get("abastecimento_excluir_confirmar_id") == int(r["id"]):
                            st.warning("Confirma a exclusão deste abastecimento?")
                            col_abs_conf, col_abs_cancel = st.columns(2)
                            if col_abs_conf.button(
                                "✅ Confirmar exclusão", type="primary", use_container_width=True,
                                key=f"btn_abastecimento_confirmar_exclusao_{int(r['id'])}",
                            ):
                                with conn() as c:
                                    c.execute("DELETE FROM abastecimentos WHERE id=?", (int(st.session_state.abastecimento_id_editando),))
                                limpar_cache_app()
                                st.session_state["abastecimento_excluir_confirmar_id"] = None
                                alerta_gravado("✅ Abastecimento excluído com sucesso!")
                                st.session_state.abastecimento_id_editando = None
                                st.rerun()
                            if col_abs_cancel.button(
                                "❌ Cancelar", use_container_width=True,
                                key=f"btn_abastecimento_cancelar_exclusao_{int(r['id'])}",
                            ):
                                st.session_state["abastecimento_excluir_confirmar_id"] = None
                                st.rerun()
        else:
...
with aba10:
    if usuario_pode_ver_aba("cadastro"):
            ...
            if placa_param_cadastro:
                row_param_placa = c.execute(
                    """SELECT consumo, manut, pneu, depre, motora_fixo, motora_pct,
                              ...
                              valor_inss, valor_acerto_folha, adiantamento_salario,
        ...
        c6, c7, c8, c9, c10, c11, c12, c13, c14, c15, c16, c17 = st.columns(12)
        ...
        adiantamento_salario_v = c13.number_input("Adiantamento Salário (R$)", value=float(dados_para_exibir.get('adiantamento_salario', 0.0)), step=10.0, key=f"adiantamento_salario_{tipo_modo}", disabled=not st.session_state.param_editando)
        esc_v = c14.number_input("Escritório (R$)", value=float(dados_para_exibir.get('cmp_custo_escritorio', 0.0)), step=50.0, key=f"esc_{tipo_modo}", disabled=not st.session_state.param_editando)
        rastreador_v = c15.number_input("VL Custo Rastreador (R$)", value=float(dados_para_exibir.get('vl_custo_rastreador', 0.0)), step=50.0, key=f"rastreador_{tipo_modo}", disabled=not st.session_state.param_editando)
        ipva_v = c16.number_input("Pagto IPVA Anual (R$)", value=float(dados_para_exibir.get('pagto_ipva', 0.0)), step=50.0, key=f"ipva_{tipo_modo}", disabled=not st.session_state.param_editando)
        imp_v = c17.number_input("% de Impostos", min_value=0.0, step=0.1, value=float(dados_para_exibir.get('imposto_pct', 0.0)), key=f"imposto_{tipo_modo}", disabled=not st.session_state.param_editando)
        ...
        if st.button(btn_label, use_container_width=True, type="primary", key=f"btn_param_save_{tipo_modo}", disabled=(not st.session_state.param_editando or not placa_param_cadastro)):
            ...
            novos_dados = {
                ...
                'valor_inss': inss_v, 'valor_acerto_folha': acerto_folha_v, 'adiantamento_salario': adiantamento_salario_v,
                    ...
                    c.execute(
                        """INSERT INTO parametros_historico (
                           ...
                           ON CONFLICT(veiculo_placa, vigencia_data) DO UPDATE SET
                               ...
                               adiantamento_salario=excluded.adiantamento_salario,
                        ...
                        (
                            ...
                            seg_v, seg_vida_motorista_v, fin_v, ipva_v, inss_v, acerto_folha_v, adiantamento_salario_v, esc_v, rastreador_v, imp_v, frete_mensal_fixo_v,
        ...
        if df_hist_param.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "adiantamento_salario": st.column_config.NumberColumn("Adiantamento Salário", format="R$ %.2f"),
            ...
            if hist_sel_label:
                ...
                h11, h12, h13, h14, h15, h16, h17, h18 = st.columns(8)
                ...
                novo_adiantamento_salario = h14.number_input("Adiantamento Salário (R$)", value=float(row_hist["adiantamento_salario"] or 0.0), step=10.0, key=f"hist_adiantamento_salario_{id_hist_sel}")
                novo_escr = h15.number_input("Escritório (R$)", value=float(row_hist["cmp_custo_escritorio"]), step=50.0, key=f"hist_esc_{id_hist_sel}")
                novo_rastreador = h16.number_input("VL Custo Rastreador (R$)", value=float(row_hist["vl_custo_rastreador"] or 0.0), step=50.0, key=f"hist_rastreador_{id_hist_sel}")
                novo_imp = h17.number_input("Imposto (%)", value=float(row_hist["imposto_pct"]), step=0.1, key=f"hist_imp_{id_hist_sel}")
                novo_frete_fixo = h18.number_input("Frete Fixo Mensal (R$)", value=float(row_hist["valor_frete_mensal_fixo"]), step=100.0, key=f"hist_ff_{id_hist_sel}")
                h19, h20 = st.columns(2)
                nova_qtde_pneu = h19.number_input("Qtde Pneu", value=float(row_hist["qtde_pneu"] or 0.0), min_value=0.0, step=1.0, format="%.0f", key=f"hist_qpneu_{id_hist_sel}")
                novo_vl_pneu_km = h20.number_input("Valor Pneu Km (R$)", value=float(row_hist["vl_gasto_pneu_km"] or 0.0), min_value=0.0, step=0.0001, format="%.4f", key=f"hist_vlpkm_{id_hist_sel}")
                        ...
                        if existe_dup:
                            ...
                            c.execute(
                                ...
                                (
                                    ...
                                    float(row_hist["adiantamento_salario"] or 0.0),
                        ...
                        if existe_data:
                            ...
                            c.execute(
                                ...
                                (
                                    ...
                                    novo_seguro, novo_seguro_vida_motorista, novo_fin, novo_ipva, novo_inss, novo_acerto_folha, novo_adiantamento_salario, novo_escr, novo_rastreador, novo_imp, novo_frete_fixo,
...
with aba12:
    if usuario_pode_ver_aba("cadastro"):
        ...
        df_veic = filtrar_df_por_placas_liberadas(df_veic, "placa")
    ...
    # =========================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba13:
    if usuario_pode_ver_aba("trocas"):
        ...
        with col_hist:
            ...
            df_t = filtrar_df_por_placas_liberadas(_carregar_controle_trocas_raw(), "veiculo_placa")
                    ...
                    with st.expander(exp_title):
                        if not st.session_state[ed_key_t]:
                            ...
                            if b2.button("🗑️ EXCLUIR", key=f"btn_del_t_{r['id']}", type="primary", use_container_width=True):
                                st.session_state["troca_excluir_confirmar_id"] = int(r["id"])
                                st.rerun()

                        if st.session_state.get("troca_excluir_confirmar_id") == int(r["id"]):
                            st.warning("Confirma a exclusão deste registro de troca?")
                            col_troca_conf, col_troca_cancel = st.columns(2)
                            if col_troca_conf.button(
                                "✅ Confirmar exclusão", type="primary", use_container_width=True,
                                key=f"btn_troca_confirmar_exclusao_{int(r['id'])}",
                            ):
                                with conn() as c:
                                    c.execute("DELETE FROM controle_trocas WHERE id=?", (int(r["id"]),))
                                limpar_cache_trocas()
                                st.session_state["troca_excluir_confirmar_id"] = None
                                alerta_gravado("✅ Registro excluído com sucesso!")
                                st.rerun()
                            if col_troca_cancel.button(
                                "❌ Cancelar", use_container_width=True,
                                key=f"btn_troca_cancelar_exclusao_{int(r['id'])}",
                            ):
                                st.session_state["troca_excluir_confirmar_id"] = None
                                st.rerun()
                        else:
    ...
    # =========================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba14:
    if usuario_pode_ver_aba("frete_liquido"):
        ...
        df_viagens_liq = filtrar_df_por_placas_liberadas(df_viagens_liq, "veiculo_placa")
        df_abs_periodo = filtrar_df_por_placas_liberadas(df_abs_periodo, "veiculo_placa")
        df_abs_ref = filtrar_df_por_placas_liberadas(df_abs_ref, "veiculo_placa")
    ...
    if usuario_pode_ver_aba("cadastro"):
        ...
        with st.expander("➕ Cadastro de Fornecedor", expanded=False):
            ...
            with st.form("form_fornecedor", clear_on_submit=True):
                c1, c2, c3 = st.columns(3)
                c1.text_input("Código Fornecedor", value=proximo_codigo, disabled=True, help="Gerado automaticamente (auto incremento).")
                cod_forn = proximo_codigo
        ...
        if st.session_state.fornecedor_editando and not df_forn.empty:
            ...
            if b_forn_excluir.button("🗑️ Deletar Selecionados", key="btn_fornecedor_deletar_selecionados", type="primary", use_container_width=True):
                ...
                if not ids_excluir_forn:
                    ...
                    st.session_state["fornecedores_excluir_confirmar_ids"] = ids_excluir_forn
                    st.rerun()

            if st.session_state.get("fornecedores_excluir_confirmar_ids"):
                qtd_forn_excluir = len(st.session_state["fornecedores_excluir_confirmar_ids"])
                st.warning(f"Confirma a exclusão de {qtd_forn_excluir} fornecedor(es)? Essa ação não pode ser desfeita.")
                col_forn_conf, col_forn_cancel = st.columns(2)
                if col_forn_conf.button("✅ Confirmar exclusão", type="primary", use_container_width=True, key="btn_fornecedor_confirmar_exclusao"):
                    bloqueados = []
                    excluidos = 0
                    with conn() as c:
                        for fornecedor_id in st.session_state["fornecedores_excluir_confirmar_ids"]:
                            fornecedor_id = int(fornecedor_id)
                            usos = movimentos_fornecedor(c, fornecedor_id)
                            linha_forn = df_forn_ed2[df_forn_ed2["id"] == fornecedor_id]
                            rotulo_forn = (
                                f"{linha_forn.iloc[0]['codigo']} - {linha_forn.iloc[0]['nome']}"
                                if not linha_forn.empty else f"ID {fornecedor_id}"
                            )
                            if usos:
                                bloqueados.append(f"{rotulo_forn} ({'; '.join(usos)})")
                                continue
                            c.execute("DELETE FROM fornecedores WHERE id=?", (fornecedor_id,))
                            excluidos += 1

                    limpar_cache_bootstrap()
                    st.session_state["fornecedores_excluir_confirmar_ids"] = None
                    if excluidos:
                    ...
                    st.rerun()
                if col_forn_cancel.button("❌ Cancelar", use_container_width=True, key="btn_fornecedor_cancelar_exclusao"):
                    st.session_state["fornecedores_excluir_confirmar_ids"] = None
                    st.rerun()
                    st.rerun()
                        ...
                        with conn() as c:
                            ...
                            p_hist_base = c.execute(
                                """SELECT consumo, manut, pneu, depre, motora_fixo, motora_pct, seguro, seguro_vida_motorista, financiamento,
                                          pagto_ipva, valor_inss, valor_acerto_folha, adiantamento_salario, vl_custo_rastreador, imposto_pct, valor_frete_mensal_fixo, qtde_pneu, vl_gasto_pneu_km
                            ...
                            c.execute(
                                ...
                                (
                                    ...
                                    float(p_hist_base["adiantamento_salario"] or 0.0),
...
with aba22:
    if usuario_pode_ver_aba("cadastro"):
        ...
        if st.session_state.cliente_id_editando is None:
            with st.form("cli_f", clear_on_submit=True):
                desc_cli = st.text_input("Descrição Cliente", key="cli_desc_novo").strip().upper()
            ...
            st.info("✏️ Você está editando um cliente. Finalize ou cancele a edição abaixo para cadastrar um novo.")
            ...
            if st.session_state.cliente_id_editando is not None:
                ...
                if reg_sel_cli.empty:
                    ...
                    with st.form("form_edicao_cliente"):
                        desc_cli_ed = st.text_input(
                            "Descrição Cliente",
                            value=str(r_cli["descricao"] or ""),
                            key=f"cliente_desc_ed_{int(r_cli['id'])}",
                        ).strip().upper()
                    ...
                    if btn_excluir_cliente:
                        st.session_state["cliente_excluir_confirmar_id"] = int(st.session_state.cliente_id_editando)
                        st.rerun()

                    if st.session_state.get("cliente_excluir_confirmar_id") == int(r_cli["id"]):
                        st.warning(f"Confirma a exclusão do cliente '{r_cli['descricao']}'?")
                        col_cli_conf, col_cli_cancel = st.columns(2)
                        if col_cli_conf.button("✅ Confirmar exclusão", type="primary", use_container_width=True, key="btn_cliente_confirmar_exclusao"):
                            with conn() as c:
                                c.execute("DELETE FROM clientes WHERE id=?", (int(st.session_state.cliente_id_editando),))
                            st.session_state["cliente_excluir_confirmar_id"] = None
                            alerta_gravado("✅ Cliente excluído com sucesso!")
                            st.session_state.cliente_id_editando = None
                            st.rerun()
                        if col_cli_cancel.button("❌ Cancelar", use_container_width=True, key="btn_cliente_cancelar_exclusao"):
                            st.session_state["cliente_excluir_confirmar_id"] = None
                            st.rerun()
...
with aba23:
    if usuario_pode_ver_aba("cadastro"):
        ...
        if st.session_state.categoria_id_editando is None:
            with st.form("cat_f", clear_on_submit=True):
                desc_cat = st.text_input("Descrição Categoria", key="cat_desc_novo").strip()
            ...
            st.info("✏️ Você está editando uma categoria. Finalize ou cancele a edição abaixo para cadastrar uma nova.")
        ...
        if df_categorias.empty:
            ...
            if c_cat3.button("🗑️ Excluir", key="btn_categoria_excluir_direto", use_container_width=True):
                st.session_state["categoria_excluir_confirmar_id"] = int(id_categoria_sel)
                st.rerun()

            if st.session_state.get("categoria_excluir_confirmar_id") in df_categorias["id"].tolist():
                id_cat_confirmar = st.session_state["categoria_excluir_confirmar_id"]
                nome_cat_confirmar = df_categorias.loc[df_categorias["id"] == id_cat_confirmar, "descricao"].iloc[0]
                st.warning(f"Confirma a exclusão da categoria '{nome_cat_confirmar}'?")
                col_cat_conf, col_cat_cancel = st.columns(2)
                if col_cat_conf.button("✅ Confirmar exclusão", type="primary", use_container_width=True, key="btn_categoria_confirmar_exclusao"):
                    with conn() as c:
                        c.execute("DELETE FROM categorias WHERE id=?", (int(id_cat_confirmar),))
                    limpar_cache_bootstrap()
                    st.session_state["categoria_excluir_confirmar_id"] = None
                    st.session_state.categoria_id_editando = None
                    alerta_gravado("✅ Categoria excluída com sucesso!")
                    st.rerun()
                if col_cat_cancel.button("❌ Cancelar", use_container_width=True, key="btn_categoria_cancelar_exclusao"):
                    st.session_state["categoria_excluir_confirmar_id"] = None
                    st.rerun()
            ...
            if st.session_state.categoria_id_editando is not None:
                ...
                if reg_sel_cat.empty:
                    ...
                    with st.form("form_edicao_categoria"):
                        desc_cat_ed = st.text_input(
                            "Descrição Categoria",
                            value=str(r_cat["descricao"] or ""),
                            key=f"categoria_desc_ed_{int(r_cat['id'])}",
                        ).strip()
                    ...
                    if btn_excluir_categoria:
                        st.session_state["categoria_excluir_confirmar_id"] = int(st.session_state.categoria_id_editando)
                        st.rerun()
...
with aba17:
    if usuario_pode_ver_aba("contas_pagar"):
        ...
        df_cp = _carregar_contas_pagar_raw()
        df_cp = filtrar_df_por_placas_liberadas(df_cp, "veiculo_placa")
        ...
        with st.expander("➕ Nova Conta a Pagar", expanded=df_cp.empty if not df_cp.empty else True):
            with st.form("form_nova_cp", clear_on_submit=True):
                ...
                opcoes_fornecedor_cp = [str(f["nome"]).strip() for f in fornecedores_db if str(f.get("nome") or "").strip()]
                cp_fornecedor = fa2.selectbox(
                    "Fornecedor / Beneficiário",
                    options=opcoes_fornecedor_cp,
                    index=None,
                    placeholder="Selecione o fornecedor",
                    key="cp_fornecedor_form",
                ) or ""
        ...
        if not df_cp.empty:
            ...
            df_cp_status_sel = df_cp.copy()
            if "Pendentes" in cp_filtro_status:
                df_cp_status_sel = df_cp_status_sel[df_cp_status_sel["status"] == "PENDENTE"]
            elif "Vencidas" in cp_filtro_status:
                df_cp_status_sel = df_cp_status_sel[df_cp_status_sel["status"] == "VENCIDO"]
            elif "Pagas" in cp_filtro_status:
                df_cp_status_sel = df_cp_status_sel[df_cp_status_sel["status"] == "PAGO"]

            df_cp_f = df_cp_status_sel.copy()
            ...
            st.markdown("---")
            st.markdown("#### 🔁 Replicar Lançamentos para o Próximo Mês")
            st.caption(
                "Marque os lançamentos que se repetem todo mês (aluguel, salário, seguro, etc.) e replique de uma vez só. "
                "As datas de Emissão e Vencimento avançam 1 mês; os demais dados (valor, categoria, fornecedor, placa) são copiados iguais "
                "e o novo lançamento entra como pendente (sem data de pagamento)."
            )
            st.caption(f"Mostrando apenas os lançamentos com status **{cp_filtro_status}** (independente dos demais filtros acima).")
            mapa_cp_replica = {
                (
                    f"ID {int(r['id'])} | {str(r.get('descricao') or '')} | Venc. "
                    f"{r['data_vencimento'].strftime('%d/%m/%Y') if pd.notna(r['data_vencimento']) else '-'} | {brl(float(r.get('valor') or 0))}"
                ): int(r["id"])
                for _, r in df_cp_status_sel.sort_values(by="id", ascending=False).iterrows()
            }
            cp_replica_sel = st.multiselect(
                "Marque os lançamentos para replicar",
                options=list(mapa_cp_replica.keys()),
                key="cp_replica_marcados",
            )
            if st.button("🔁 Replicar Marcados para o Próximo Mês", key="btn_cp_replicar_marcados", use_container_width=True):
                if not cp_replica_sel:
                    st.warning("Marque pelo menos um lançamento para replicar.")
                else:
                    ids_cp_replica = [mapa_cp_replica[x] for x in cp_replica_sel]
                    novos_ids_cp = []
                    with conn() as c:
                        placeholders_cp_replica = ",".join(["?"] * len(ids_cp_replica))
                        linhas_replica_cp = c.execute(
                            f"SELECT * FROM contas_pagar WHERE id IN ({placeholders_cp_replica})",
                            ids_cp_replica,
                        ).fetchall()
                        for linha_raw in linhas_replica_cp:
                            linha = dict(linha_raw)
                            nova_emissao = somar_um_mes(linha.get("data_emissao"))
                            nova_vencimento = somar_um_mes(linha.get("data_vencimento"))
                            c.execute(
                                """INSERT INTO contas_pagar
                                   (descricao, fornecedor, categoria, n_documento, data_emissao,
                                    data_vencimento, valor, data_pagamento, forma_pagamento, observacao, data_cadastro, veiculo_placa)
                                ...
                                (
                                    ...
                                    linha.get("categoria"),
                                    linha.get("n_documento"),
                                    nova_emissao.isoformat() if nova_emissao else None,
                                    nova_vencimento.isoformat() if nova_vencimento else None,
                                    linha.get("valor"),
                                    None,
                                    linha.get("forma_pagamento"),
                                    linha.get("observacao"),
                                    datetime.now().isoformat(),
                                    linha.get("veiculo_placa"),
                                ),
                            )
                            novos_ids_cp.append(int(c.execute("SELECT last_insert_rowid()").fetchone()[0]))

                    # Confere no banco se os novos lançamentos realmente foram gravados antes de avisar sucesso
                    qtd_confirmada_cp = 0
                    if novos_ids_cp:
                        with conn() as c:
                            placeholders_verif_cp = ",".join(["?"] * len(novos_ids_cp))
                            qtd_confirmada_cp = c.execute(
                                f"SELECT COUNT(*) FROM contas_pagar WHERE id IN ({placeholders_verif_cp})",
                                novos_ids_cp,
                            ).fetchone()[0]

                    if qtd_confirmada_cp > 0 and qtd_confirmada_cp == len(ids_cp_replica):
                        alerta_gravado(f"✅ {qtd_confirmada_cp} lançamento(s) replicado(s) com sucesso para o próximo mês!")
                        st.rerun()
                    elif qtd_confirmada_cp > 0:
                        limpar_cache_app()
                        st.warning(
                            f"⚠️ Apenas {qtd_confirmada_cp} de {len(ids_cp_replica)} lançamento(s) foram gravados. "
                            "Confira a lista abaixo e replique novamente os que faltaram, se necessário."
                        )
                        st.rerun()
                    else:
                        st.error("❌ Não foi possível confirmar a gravação dos lançamentos replicados. Nada foi salvo — tente novamente.")
            ...
            opcoes_cp_sel = {
                ...
                for _, r in df_cp_status_sel.sort_values("data_emissao", na_position="last").iterrows()
            }
            ...
            if cp_sel_label:
                ...
                if st.session_state.get("cp_duplicando_id") == cp_id_sel:
                    ...
                    with st.form(f"form_dup_cp_{cp_id_sel}"):
                        ...
                        cp_d_forn_atual = str(cp_row.get("fornecedor") or "").strip()
                        opcoes_forn_dup_cp = [str(f["nome"]).strip() for f in fornecedores_db if str(f.get("nome") or "").strip()]
                        if cp_d_forn_atual and cp_d_forn_atual not in opcoes_forn_dup_cp:
                            opcoes_forn_dup_cp = [cp_d_forn_atual] + opcoes_forn_dup_cp
                        cp_d_forn = d2.selectbox(
                            "Fornecedor",
                            options=opcoes_forn_dup_cp,
                            index=opcoes_forn_dup_cp.index(cp_d_forn_atual) if cp_d_forn_atual in opcoes_forn_dup_cp else None,
                            placeholder="Selecione o fornecedor",
                            key=f"cp_d_forn_{cp_id_sel}",
                        ) or ""
                ...
                if st.session_state.get("cp_editando_id") == cp_id_sel:
                    ...
                    with st.form(f"form_edit_cp_{cp_id_sel}"):
                        ...
                        cp_e_forn_atual = str(cp_row.get("fornecedor") or "").strip()
                        opcoes_forn_edit_cp = [str(f["nome"]).strip() for f in fornecedores_db if str(f.get("nome") or "").strip()]
                        if cp_e_forn_atual and cp_e_forn_atual not in opcoes_forn_edit_cp:
                            opcoes_forn_edit_cp = [cp_e_forn_atual] + opcoes_forn_edit_cp
                        cp_e_forn = e2.selectbox(
                            "Fornecedor",
                            options=opcoes_forn_edit_cp,
                            index=opcoes_forn_edit_cp.index(cp_e_forn_atual) if cp_e_forn_atual in opcoes_forn_edit_cp else None,
                            placeholder="Selecione o fornecedor",
                            key=f"cp_e_forn_{cp_id_sel}",
                        ) or ""
    ...
    # =========================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba_cr:
    if usuario_pode_ver_aba("contas_receber"):
        ...
        df_cr = _carregar_contas_receber_raw()
        df_cr = filtrar_df_por_placas_liberadas(df_cr, "veiculo_placa")
            ...
            if cr_sel_label:
                ...
                if st.session_state.get("cr_duplicando_id") == cr_id_sel:
                    ...
                    with st.form(f"form_dup_cr_{cr_id_sel}"):
                        ...
                        cr_d_cli_atual = str(cr_row.get("cliente") or "").strip()
                        opcoes_cli_dup_cr = list(lista_clientes) if isinstance(lista_clientes, list) else []
                        if cr_d_cli_atual and cr_d_cli_atual not in opcoes_cli_dup_cr:
                            opcoes_cli_dup_cr = [cr_d_cli_atual] + opcoes_cli_dup_cr
                        cr_d_cli = dr2.selectbox(
                            "Cliente",
                            options=opcoes_cli_dup_cr,
                            index=opcoes_cli_dup_cr.index(cr_d_cli_atual) if cr_d_cli_atual in opcoes_cli_dup_cr else None,
                            placeholder="Selecione o cliente",
                            key=f"cr_d_cli_{cr_id_sel}",
                        ) or ""
                ...
                if st.session_state.get("cr_editando_id") == cr_id_sel:
                    ...
                    with st.form(f"form_edit_cr_{cr_id_sel}"):
                        ...
                        cr_e_cli_atual = str(cr_row.get("cliente") or "").strip()
                        opcoes_cli_edit_cr = list(lista_clientes) if isinstance(lista_clientes, list) else []
                        if cr_e_cli_atual and cr_e_cli_atual not in opcoes_cli_edit_cr:
                            opcoes_cli_edit_cr = [cr_e_cli_atual] + opcoes_cli_edit_cr
                        cr_e_cli = re2.selectbox(
                            "Cliente",
                            options=opcoes_cli_edit_cr,
                            index=opcoes_cli_edit_cr.index(cr_e_cli_atual) if cr_e_cli_atual in opcoes_cli_edit_cr else None,
                            placeholder="Selecione o cliente",
                            key=f"cr_e_cli_{cr_id_sel}",
                        ) or ""
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba_fluxo:
    if usuario_pode_ver_aba("fluxo_caixa"):
        ...
        if fluxo_ini > fluxo_fim:
            ...
            df_fluxo_cp = _carregar_contas_pagar_raw()
            df_fluxo_cr = _carregar_contas_receber_raw()
            df_fluxo_cp = filtrar_df_por_placas_liberadas(df_fluxo_cp, "veiculo_placa")
            df_fluxo_cr = filtrar_df_por_placas_liberadas(df_fluxo_cr, "veiculo_placa")
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba18:
    if usuario_pode_ver_aba("me_lembra"):
    ...
    # =========================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba20:
    if usuario_pode_ver_aba("anotacoes"):
    ...
    # =========================
        ...
        st.info("🔒 Você não tem permissão para visualizar esta aba.")
with aba_calc:
    if usuario_pode_ver_aba("calculo_rapido"):
        st.markdown(
            """
            <style>
            .cmp-header {
